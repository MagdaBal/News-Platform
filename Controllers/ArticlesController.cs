    ...
    public class ArticlesController : Controller
    {
        ...
        public IActionResult GetArticle(int id)
        {
            ...
            if (LoginController.thisLogin.isL)  // Login OK
            {
                ...
                using (var context = new DbNewsContext())
                {
                    var comments = context.Comments.Where(c => c.IdArticle == articles[id].Id).OrderByDescending(c => c.Content);
 
                    TempData["CommQty"] = comments.Count();   // number of comments
                    int i = 0;
                    foreach(var comment in comments)
                    {
                        TempData["Comm" + i.ToString()] = comment.Content;
                    }
                }
        ...
        {
            string[] _negativeWords = { "worst", "bad", "unfamiliar", "unhappy", "lost", "failure" };
            string[] _positiveWords = { "win", "won", "achieve", "achieved", "great", "super", "sucess" };
